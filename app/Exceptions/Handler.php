...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if($exception instanceof MethodNotAllowedHttpException) {
            return response()->json(['code'=>405, 'error'=> true, 'message' => 'Method is not allowed!']);
        }

        if($exception instanceof NotFoundHttpException){
            return response()->json(['code'=>404, 'error'=> true, 'message' => 'URL Not Found!']);
        }

        if($exception instanceof QueryException){
            return response()->json(['code'=>500, 'error'=> true, 'message' => 'Internal Server Error!']);
        }
        ...
        if($exception instanceof InvalidArgumentException){
            return response()->json(['code'=>400, 'error'=> true, 'message' => 'You provided some invalid input value!']);
        }
    ...
    protected function formatException(\Throwable $exception): array
    {
        ...
        if(! empty($definition['adaptMessage'])) {
            ...
            return $definition['message'] = $exception->getMessage() ?? $definition['message'];
